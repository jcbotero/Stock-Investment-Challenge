...
const nutresaaE = [
 ...
 export default function Escenario5({idioma, setPortaec5,setPortaBan5, setPortaCem5,setPortaNut5, setEcova, setBanva,setCemva,setNutva, cantiEco,cantiBan,cantiCem,cantiNut,  setEcova2,setBanva2,setCemva2,setNutva2, cantiEco2, cantiBan2,cantiCem2,cantiNut2, setEcova3,setBanva3,setCemva3,setNutva3, cantiEco3, cantiBan3,cantiCem3,cantiNut3,setEcova4,setBanva4,setCemva4,setNutva4, cantiEco4, cantiBan4,cantiCem4,cantiNut4, setEcova5,setBanva5,setCemva5,setNutva5, cantiEco5, cantiBan5,cantiCem5,cantiNut5    } ) { // aqui estoy recibiendo las props enviadas desde el parent App
  ...
  const [noticias, setNoticias] = useState(data[4])
  ...
  useEffect(()=>{
    ...
    if ( idioma == "español") {
     ...
     switch (data[4].general) { // casos para poder saber el escenario economico y asi darle valor a las acciones
   ...
   if ( nutett <= 2 ) {
    ...
    setAcciones("STOCK PRICES")
      ...
      switch (dataEn[4].general) { // casos para poder saber el escenario economico y asi darle valor a las acciones
    ...
    const grutt =  grupall.indexOf(grupalNews[3]); // aqui estoy encontrando el index de la noticia grupal en la array original para poder dar un valor y alterar el precio de la accion de eco
    // estas constantes van con el index [0] por que el precio depende de la noticia del escenario anterior
   

    const gruttE =  grupalEn.indexOf(grupalNewsEn[3]); // lo mismo de arriba pero en ingles


    const ecott =  ecoo.indexOf(ecopetrol[3]);   
    const bancott =  bcoll.indexOf(bancolombia[3]);
    const cematt =  cargoss.indexOf(cemargos[3]);
    const nutett =  nutresaa.indexOf(gNutresa[3]);

    const ecottE =  ecooE.indexOf(ecopetrolEn[3]);   
    const bancottE =  bcollE.indexOf(bancolombiaEn[3]);
    const cemattE =  cargossE.indexOf(cemargosEn[3]);
    const nutettE =  nutresaaE.indexOf(gNutresaEn[3]);  // lo mismo de arriba pero en ingles
