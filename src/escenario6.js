...
const nutresaaE = [
 ...
 export default function Escenario6({props,idioma, setPortaec6,setPortaBan6, setPortaCem6,setPortaNut6, setEcova, setBanva,setCemva,setNutva, cantiEco,cantiBan,cantiCem,cantiNut,  setEcova2,setBanva2,setCemva2,setNutva2, cantiEco2, cantiBan2,cantiCem2,cantiNut2, setEcova3,setBanva3,setCemva3,setNutva3, cantiEco3, cantiBan3,cantiCem3,cantiNut3,setEcova4,setBanva4,setCemva4,setNutva4, cantiEco4, cantiBan4,cantiCem4,cantiNut4, setEcova5,setBanva5,setCemva5,setNutva5, cantiEco5, cantiBan5,cantiCem5,cantiNut5, setEcova6,setBanva6,setCemva6,setNutva6, cantiEco6, cantiBan6,cantiCem6,cantiNut6    } ) { // aqui estoy recibiendo las props enviadas desde el parent App
  ...
  const [noticias, setNoticias] = useState(data[5])
  ...
  if  ( idioma == "español") {
     ...
     switch (data[5].general) { // casos para poder saber el escenario economico y asi darle valor a las acciones
   ...
   if ( nutett <= 2 ) {
    ...
    setAcciones("STOCK PRICES")
      ...
      switch (dataEn[5].general) { // casos para poder saber el escenario economico y asi darle valor a las acciones
    ...
    const grutt =  grupall.indexOf(grupalNews[4]); // aqui estoy encontrando el index de la noticia grupal en la array original para poder dar un valor y alterar el precio de la accion de eco
    // estas constantes van con el index [0] por que el precio depende de la noticia del escenario anterior
   

    const gruttE =  grupalEn.indexOf(grupalNewsEn[4]); // lo mismo de arriba pero en ingles


    const ecott =  ecoo.indexOf(ecopetrol[4]);   
    const bancott =  bcoll.indexOf(bancolombia[4]);
    const cematt =  cargoss.indexOf(cemargos[4]);
    const nutett =  nutresaa.indexOf(gNutresa[4]);

    const ecottE =  ecooE.indexOf(ecopetrolEn[4]);   
    const bancottE =  bcollE.indexOf(bancolombiaEn[4]);
    const cemattE =  cargossE.indexOf(cemargosEn[4]);
    const nutettE =  nutresaaE.indexOf(gNutresaEn[4]);  // lo mismo de arriba pero en ingles
