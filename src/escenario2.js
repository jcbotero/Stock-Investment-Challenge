...
const nutresaaE = [
 ...
 export default function Escenario2( {setPortaec2, idioma,  setPortaBan2, setPortaCem2,setPortaNut2, setEcova, setBanva,setCemva,setNutva, cantiEco,cantiBan,cantiCem,cantiNut,  setEcova2,setBanva2,setCemva2,setNutva2, cantiEco2, cantiBan2,cantiCem2,cantiNut2  } ) { // aqui estoy recibiendo las props enviadas desde el parent App
   ...
   // english section 
  const [noticias, setNoticias] = useState(data[1])
  ...
  const [acciones,setAcciones] = useState("PRECIOS ACCIONES")
   ...
   if ( idioma == "español") {
     ...
     switch (data[1].general) { // casos para poder saber el escenario economico y asi darle valor a las acciones
   ...
   if ( nutett <= 2 ) {
    ...
    setAcciones("STOCK PRICES")
      ...
      switch (dataEn[1].general) { // casos para poder saber el escenario economico y asi darle valor a las acciones
    ...
    const gruttE =  grupalEn.indexOf(grupalNewsEn[0]); // lo mismo de arriba pero en ingles
    ...
    const ecottE =  ecooE.indexOf(ecopetrolEn[0]);   
    const bancottE =  bcollE.indexOf(bancolombiaEn[0]);
    const cemattE =  cargossE.indexOf(cemargosEn[0]);
    const nutettE =  nutresaaE.indexOf(gNutresaEn[0]);  // lo mismo de arriba pero en ingles
