...
const nutresaaE = [
 ...
 export default function Escenario4 ({ idioma,setPortaec4,setPortaBan4, setPortaCem4,setPortaNut4, setEcova, setBanva,setCemva,setNutva, cantiEco,cantiBan,cantiCem,cantiNut,  setEcova2,setBanva2,setCemva2,setNutva2, cantiEco2, cantiBan2,cantiCem2,cantiNut2, setEcova3,setBanva3,setCemva3,setNutva3, cantiEco3, cantiBan3,cantiCem3,cantiNut3,setEcova4,setBanva4,setCemva4,setNutva4, cantiEco4, cantiBan4,cantiCem4,cantiNut4   } ) { // aqui estoy recibiendo las props enviadas desde el parent App
  ...
  const [noticias, setNoticias] = useState(data[3])
  ...
  useEffect(()=>{
    ...
    if ( idioma == "español") {
     ...
     switch (data[3].general) { // casos para poder saber el escenario economico y asi darle valor a las acciones
   ...
   if ( nutett <= 2 ) {
    ...
    setAcciones("STOCK PRICES")
      ...
      switch (dataEn[3].general) { // casos para poder saber el escenario economico y asi darle valor a las acciones
    ...
    const grutt =  grupall.indexOf(grupalNews[2]); // aqui estoy encontrando el index de la noticia grupal en la array original para poder dar un valor y alterar el precio de la accion de eco
    // estas constantes van con el index [0] por que el precio depende de la noticia del escenario anterior
   

    const gruttE =  grupalEn.indexOf(grupalNewsEn[2]); // lo mismo de arriba pero en ingles


    const ecott =  ecoo.indexOf(ecopetrol[2]);   
    const bancott =  bcoll.indexOf(bancolombia[2]);
    const cematt =  cargoss.indexOf(cemargos[2]);
    const nutett =  nutresaa.indexOf(gNutresa[2]);

    const ecottE =  ecooE.indexOf(ecopetrolEn[2]);   
    const bancottE =  bcollE.indexOf(bancolombiaEn[2]);
    const cemattE =  cargossE.indexOf(cemargosEn[2]);
    const nutettE =  nutresaaE.indexOf(gNutresaEn[2]);  // lo mismo de arriba pero en ingles
